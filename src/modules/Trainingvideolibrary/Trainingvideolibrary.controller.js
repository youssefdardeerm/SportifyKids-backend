...
import { TrainingVideoLibrarymodel } from '../../../db/models/Trainingvideolibrary.model.js';
...
export const uploadVideo = async (req, res) => {
  try {
    ...
    const newVideo = new TrainingVideoLibrarymodel({ url });
    await newVideo.save();
...
export const getVideos = async (req, res) => {
    ...
    try {
      const videos = await TrainingVideoLibrarymodel.find().skip(skip).limit(limit); // استرجاع الفيديوهات الحالية
      const totalVideos = await TrainingVideoLibrarymodel.countDocuments(); // إجمالي عدد الفيديوهات
